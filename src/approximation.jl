...
`method` is the method of the approximation. This can be set to `:binary`, `:echelon`, `:convex`,
`:bisection`, `:SOS1` or `:SOS2`.

`type` specifies how the approximation bounds the actual function. This can be set to `:upper`,
`:lower`, `:interior`, `tangent_cuts` or `:combined`.

`knots` must list the function's discontinuities, and points of inflection. Can be provided with
curvature specifed, but if not this will be determined automatically.
...
function approximate(
    ...
    xi, fxi = find_points(func, n, δ, knots, knots_shape, type)
    n = length(xi) - 1
