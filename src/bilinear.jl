"""
    one_binary_bilinear_formulation(
        model::JuMP.Model,
        xy::VariableRef,
...
function discrete_bilinear_formulation(
    ...
    if length(x_values) >= 50
        @warn(
            "The number of discrete levels being modelled ($(length(x_values))) within the bilinear
            function is large, consider using an approximation"
        )
    end
    ϵ[0] = 0.0
...
Function that defines a bilinear formulation for a given pair of variables or affine expressions.
This function will select the best formulation, given `x` and `y`.

### Required arguments
`x` and `y` are either of type `VariableRef` or `AffExpr` and `xy` is the product that is being
modelled.

### Optional arguments
`n` is used if the product is approximated using the difference of two squares.

`method` is used if the product is approximated using the difference of two squares. This can be set
to `:binary`, `:echelon`, `:SOS1` or `:SOS2`.

`type` is used if the product is approximated using the difference of two squares. This can be set
to `:upper`, `:lower`, `:interior`, `cutting_planes` or `:combined`.
...
Function that defines a power formulation (`x^y`) for a given pair of variables or affine expressions.
This function will select the best formulation, given `x` and `y`.

### Required arguments
`x` and `y` are either of type `VariableRef` or `AffExpr` and `x^y` is the power function that
is being modelled.

### Optional arguments
`n` is used if the power is approximated using the difference of two squares.

`method` is used if the power is approximated using the difference of two squares. This can be set
to `:binary` `:echelon` `:SOS1` or `:SOS2`.

`type` is used if the power is approximated using the difference of two squares. This can be set
to `:upper` `:lower` `:interior` `cutting_planes` or `:combined`.
