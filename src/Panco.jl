...
function bilinear(x::Union{VariableRef,AffExpr}, y::Union{VariableRef,AffExpr}, n::Int)
    ...
    a² = linearise(((x-(lx+ux)/2)/(ux-lx)+(y-(ly+uy)/2)/(uy-ly))/2, x -> x^2, 2*n)
    b² = linearise(((x-(lx+ux)/2)/(ux-lx)-(y-(ly+uy)/2)/(uy-ly))/2, x -> x^2, 2*n)
    @constraint(model,xy-x*(ly+uy)/2-y*(lx+ux)/2+(lx+ux)*(ly+uy)/4==(a²-b²)*(ux-lx)*(uy-ly))
